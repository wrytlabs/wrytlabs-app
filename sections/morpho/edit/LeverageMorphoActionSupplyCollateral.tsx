...
export default function LeverageMorphoActionSupplyCollateral({ instance, amount, allowance, disabled }: Props) {
	...
	const handleOnClick = async function (e: any) {
		...
		try {
			...
			const toastContent = [
				{
					title: `Supply collateral amount: `,
					value: `${formatCurrency(formatUnits(amount, instance.collateralDecimals))} ${instance.collateralSymbol}`,
				},
				{
					title: `Instance: `,
			...
			await toast.promise(waitForTransactionReceipt(WAGMI_CONFIG, { hash: writeHash, confirmations: 1 }), {
				pending: {
					render: <TxToast title={`Supplying collateral...`} rows={toastContent} />,
				},
				success: {
					render: <TxToast title="Successfully supplied" rows={toastContent} />,
				},
			});
